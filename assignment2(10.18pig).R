...
# 'days' represents days from 1 to 310
days <- 1:310

# Plot all of variation of t0 on the same graph, using the same color (blue)
matplot(days, result$inft, type = "l", lty = 1, col = "blue", 
        xlab = "Days", ylab = "Infection Frequency", 
        main = "Incidence Trajectory Over Time")

legend("topright", legend = "Uncertainty", 
       col = "blue", lty = 1, cex = 0.5)

lines(days, result$inft[,100], col = "yellow", lwd = 1.5)

abline(v=84,lwd = 2, lty = 2,col="black")

legend("topright", legend = c("Uncertainty", "t0 converged value"), 
       col = c("blue", "yellow"), lty = 1, lwd = 2, cex = 0.5)
